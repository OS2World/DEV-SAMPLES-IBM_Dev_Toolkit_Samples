...
void TCPClient(struct sockaddr_in *server);
void UDPClient(struct sockaddr_in *server);
...
void main(int argc, char **argv) {
   ...
   // Call the correct client routine
...
void UDPClient(struct sockaddr_in *server) {
   ...
   while (1) {
      ...
      rc = sendto(sock, buf, strlen(buf), 0, (struct sockaddr *)server, sizeof(*server));
      if (rc == -1) {
         psock_errno("send()");
         exit(1);
      }
      ...
      len = recvfrom(sock, buf, sizeof(buf), 0, (struct sockaddr *)&response, &respsize);
      if (len == -1) {
         psock_errno("recv()");
         exit(1);
      }
